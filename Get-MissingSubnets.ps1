...
(	
	...
	[ValidateScript({Test-Path $_ -PathType Container})]
	[String] $Path = "$env:userprofile\Desktop\ADData\DomainControllers\MissingSubnets",
	[parameter(ParameterSetName="Domain")]
...
#Region Functions
$final_local = "$env:userprofile\Desktop\ADData\DomainControllers\MissingSubnets"
...
else
{
	if ( [string]::IsNullOrEmpty($Domain) )
	{
		...
		{
			Write-Warning "The script is going to retrieve the netlogon log file of all domain controllers in the forest, Just enter Yes."
			$Confirm = Read-Host
		}
